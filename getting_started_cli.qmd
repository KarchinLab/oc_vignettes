...
In the diagram below, we'll see the basic Open-Cravat workflow on the command-line.
...
And make a separate virtual environment using the `virtualenv` command. Here we're creating a virtual environment called `oc`:
...
```
tedladeras@teds-MacBook-Pro ~ % virtualenv oc
created virtual environment CPython3.11.7.final.0-64 in 260ms
  creator CPython3macOsBrew(dest=/Users/tedladeras/oc, clear=False, no_vcs_ignore=False, global=False)
  seeder FromAppData(download=False, pip=bundle, setuptools=bundle, wheel=bundle, via=copy, app_data_dir=/Users/tedladeras/Library/Application Support/virtualenv)
...
You can double check whether the environment is activated by using `which python3` again. Note that our prompt also has an `(oc)` in front. 
...
## Understanding the Input File Format [to be updated with new example_input]
